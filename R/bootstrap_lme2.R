...
#' @import foreach
#' @export
boot_lme2 <- function(name.test, name.class, name.covars, name.clust, data, levl.class = NULL,
                      B = 250, type = c("cluster", "stratified"), boxcox = TRUE, interval_lambda = c(-2, 2),
  ...
  bts_func <- function(B, data_list, name.test, name.class, name.covars, name.clust, levl.class, type, boxcox,
    ...
    while(flag == 0){
      ...
      out <- try(lme2(name.test = name.test, name.class = name.class, name.covars = name.covars,
                      ...
                      boxcox = boxcox, apVar = FALSE, trace = FALSE, interval_lambda = interval_lambda), silent = TRUE)
      if(class(out) != "try-error") flag <- 1
    }
    return(out$est_para)
  }
  if(!parallel){
    ...
    for(i in 1:B){
      out_boot[,i] <- bts_func(B = B, data_list = data_list, name.test = name.test, name.class = name.class,
                               ...
                               type = type, boxcox = boxcox, interval_lambda = interval_lambda)
    }
  } else{
    ...
    out_boot <- parSapply(cl, X = 1:B, FUN = bts_func, data_list = data_list, name.test = name.test,
                          ...
                          levl.class = levl.class, type = type, boxcox = boxcox, interval_lambda = interval_lambda)
    stopCluster(cl)
  }
  return(out_boot)
}
