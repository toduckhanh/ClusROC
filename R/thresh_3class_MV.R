...
MV_est_fun <- function(par_model, x.val, n_p, boxcox, par_boxcox, start = NULL, method.optim,
                       maxit, lower, upper){
  beta_d <- matrix(par_model[1:(3*n_p)], ncol = n_p, nrow = 3, byrow = TRUE)
  Z <- c(1, x.val)
  ...
  #
  MV_optim <- optim(par = start, fn = MV_normal, par_model = par_model, x.val = x.val, n_p = n_p,
